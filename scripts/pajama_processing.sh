...
export DEST_PATH=$BINS_ROOT/$DATASET_NAME

# if charpercent is not empty, add it to the destination path

export TK_PATH=/home/aiops/zhuty/tinyllama-public/tokenizer/
# export TK_PATH=/home/aiops/zhuty/temp_models/Qwen2.5-0.5B
# ensure that the SOURCE PATH and TK PATH exist
if [ ! -d $SOURCE_PATH ]; then
    echo "Error: $SOURCE_PATH does not exist"
    exit 1
fi
if [ ! -d $TK_PATH ]; then
    echo "Error: $TK_PATH does not exist"
fi

if [ $PROCESSING_LENGTH == "4k" ]; then
    # chunk_size=65552 # (4096 + 1) * 16
    chunk_size=33562624 # (4096 + 1) * 8192
    ...
    chunk_size=8396800 # (1024 + 1) * 8192
    ...
    # chunk_size=134225920 # (16384 + 1) * 8192
    chunk_size=2097280
